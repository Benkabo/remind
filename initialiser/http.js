...
const serverStaticFile = require('../lib/responder')
...
module.exports = () => {
    const server = createServer( async ({url}, response) => {
       ...
       try {
        return await serverStaticFile({ file: path, extension: isRoot ? 'html' : extension }, response)
       } catch (error) {
           console.error(error)
           return await serverStaticFile({
               file: '/error.html',
               extension: 'html',
               statusCode: 500
           }, response)
